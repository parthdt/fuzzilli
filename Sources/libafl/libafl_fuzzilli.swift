...
#if canImport(libafl_fuzzilliFFI)
import libafl_fuzzilliFFI
#endif
...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_libafl_fuzzilli_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_libafl_fuzzilli_rustbuffer_free(self, $0) }
    }
}
...
public convenience init(corpusDir: String, shmemKey: String, schedulerType: UInt8) {
    let pointer =
        try! rustCall() {
    uniffi_libafl_fuzzilli_fn_constructor_libaflobject_new(
        FfiConverterString.lower(corpusDir),
        FfiConverterString.lower(shmemKey),
        FfiConverterUInt8.lower(schedulerType),$0
    )
}
...
private var initializationResult: InitializationResult = {
    ...
    let scaffolding_contract_version = ffi_libafl_fuzzilli_uniffi_contract_version()
    ...
    if (uniffi_libafl_fuzzilli_checksum_constructor_libaflobject_new() != 54002) {
        return InitializationResult.apiChecksumMismatch
    }
